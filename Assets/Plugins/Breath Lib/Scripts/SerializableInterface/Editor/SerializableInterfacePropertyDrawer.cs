...
namespace BreathLib.SerializableInterface
{
    ...
    internal sealed class SerializableInterfacePropertyDrawer : PropertyDrawer
    {
        ...
        private IReferenceDrawer GetReferenceDrawer(SerializedProperty property, GUIContent label)
        {
			...
			SerializedProperty excludesUnityObjectsProperty = serializedProperty.FindPropertyRelative("excludesUnityObjects");
            ReferenceMode referenceMode = (ReferenceMode)modeProperty.enumValueIndex;
			bool excludesUnityObjects = excludesUnityObjectsProperty.boolValue;

            switch (referenceMode)
            {
                case ReferenceMode.Raw:
                    rawReferenceDrawer.Initialize(property, genericType, label, fieldInfo, excludesUnityObjects);
                    return rawReferenceDrawer;
                case ReferenceMode.Unity:
                    unityReferenceDrawer.Initialize(property, genericType, label, fieldInfo, excludesUnityObjects);
