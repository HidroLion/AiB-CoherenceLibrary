...
namespace BreathLib.Core
{
	public static class ConditionalUtility
	{
		public static bool IsConditionMatch(UnityEngine.Object owner, ConditionalData condition)
		{
			...
			{
				if (string.IsNullOrEmpty(fieldCondition.Field)) continue;
		...
		private static void LogFieldNotFound(SerializedProperty property, string field) => Debug.LogWarning(
			$"Conditional Attribute is trying to check field <color=brown>{field}</color> which is not present",
			property.serializedObject.targetObject);
		private static void LogFieldNotFound(UnityEngine.Object owner, string field) => Debug.LogWarning(
			$"Conditional Attribute is trying to check field <color=brown>{field}</color> which is not present",
			owner);

		public static void LogCollectionsNotSupportedWarning(UnityEditor.SerializedProperty property, string nameOfType) => Debug.LogWarning(
			$"Array fields are not supported by <color=brown>[{nameOfType}]</color>. " +
		...
		private static SerializedProperty FindRelativeProperty(SerializedProperty property, string propertyName)
		{
			...
			if (nestedProperty == null)
			{
				...
				LogCollectionsNotSupportedWarning(arrayProp, nameof(ConditionalFieldAttribute));
