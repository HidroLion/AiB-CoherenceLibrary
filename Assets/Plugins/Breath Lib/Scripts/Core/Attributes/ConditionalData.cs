...
namespace BreathLib.Core
{
	...
	{
		public bool IsSet => !string.IsNullOrEmpty(_fieldToCheck) || (_fieldsToCheckMultiple != null && _fieldsToCheckMultiple.Length > 0) || !string.IsNullOrEmpty(_predicateMethod);
		...
		{
			if (!string.IsNullOrEmpty(_fieldToCheck)) yield return (_fieldToCheck, _inverse, _compareValues);
			if ((_fieldsToCheckMultiple != null && _fieldsToCheckMultiple.Length > 0))
			{
		...
		{
			if (string.IsNullOrEmpty(_predicateMethod)) return true;
		...
		{
			if (string.IsNullOrEmpty(_predicateMethod)) return null;
			...
			{
				Debug.LogWarning(
					$"Conditional Attribute is trying to invoke method <color=brown>{_predicateMethod}</color> " +
